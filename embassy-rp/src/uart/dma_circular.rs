//! DMA-backed circular buffered UART driver for RP2350.
//!
//! This implementation uses DMA with self-triggering (RP2350 only) or dual-channel
...
pub struct DmaCircularState {
    ...
    /// Write position in the circular buffer (updated by DMA)
    write_pos: AtomicUsize,
    /// Waker for RX operations
    rx_waker: AtomicWaker,
    /// DMA channel waker
    dma_waker: AtomicWaker,
}
...
impl<'d> DmaCircularUartRx<'d, Async> {
    ...
    ///
    /// The buffer must be aligned and should be a power of 2 size for optimal performance.
    #[cfg(feature = "_rp235x")]
    pub fn new_circular<T: Instance>(
        ...
        state.write_pos.store(0, Ordering::Relaxed);
...
// Extend Instance trait to include DMA circular state
pub trait DmaCircularInstance: Instance {
    fn dma_circular_state() -> &'static DmaCircularState;
}

// This would need to be implemented in the Instance macro
// For now, this is a placeholder to check compilation
